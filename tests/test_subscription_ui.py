...
def test_subscription_page_and_assets_are_served(tmp_path):
    ...
    assert "Research Subscriptions" in page.text
    assert "New subscription" in page.text
    assert "Run Now" in script.text
    assert "/api/v1/providers" in script.text
    assert "/api/v1/subscriptions" in script.text
    assert 'button.textContent = "Running..."' in script.text
...
def test_subscription_script_uses_safe_errors_and_prevents_duplicate_submit():
    ...
    assert "Subscription unavailable" in script
    assert "Provider partial failure" in script
    assert "Run failed" in script
    assert "Invalid schedule" in script
